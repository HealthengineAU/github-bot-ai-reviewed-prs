...
// `diffSize` is known and `ai_review.provider_groups` has a band covering it,
// the pick is restricted to that band's summonable providers; otherwise every
// enabled provider is in the running.
export async function triggerRandomReviewer(octokit, { owner, repo, issue_number, config, diffSize }) {
  const enabled = AI_REVIEWER_TRIGGERS.filter((t) => config.isProviderEnabled(t.provider));
  if (enabled.length === 0) {
    return;
  }

  const group = config.providersForDiffSize?.(diffSize);
