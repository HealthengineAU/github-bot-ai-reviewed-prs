...
// Display names for specific bot logins (exact match, unlike the provider
// substring matching above). An entry here does not make a login count as an
// AI-review provider.
export const BOT_LOGIN_DISPLAY_NAMES = {
  "dusty-the-robot[bot]": "Dusty",
};
...
export function getBotDisplayName(login) {
  const exact = BOT_LOGIN_DISPLAY_NAMES[login?.toLowerCase()];
...
// True when at least one summonable reviewer (Auggie, Copilot) is enabled —
...
// The size `ai_review.provider_groups` bands are measured against. Defaults to
// the full diff; `provider_group_metric: additions` counts additions only, so
// a large deletion lands in the small band instead of the large one.
export function getProviderGroupSize(pr, aiReview) {
  return aiReview?.providerGroupMetric === "additions"
    ? (pr?.additions ?? 0)
    : getDiffSize(pr);
}

// Summon one of the enabled AI reviewers at random (Auggie or Copilot). When
// `bandSize` is known and `ai_review.provider_groups` has a band covering it,
// the pick is restricted to that band's summonable providers; otherwise every
// enabled provider is in the running.
export async function triggerRandomReviewer(octokit, { owner, repo, issue_number, config, bandSize }) {
  const enabled = AI_REVIEWER_TRIGGERS.filter((t) => config.isProviderEnabled(t.provider));
  if (enabled.length === 0) {
    return;
  }

  const group = config.providersForBandSize?.(bandSize);
  const grouped = group ? enabled.filter((t) => group.has(t.provider)) : [];
  const candidates = grouped.length > 0 ? grouped : enabled;

  const index = Math.floor(Math.random() * candidates.length);
  await candidates[index].trigger(octokit, { owner, repo, issue_number });
}
