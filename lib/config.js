...
// based on the PR's diff size, e.g. small PRs to Copilot and large ones to
// Auggie. It only ever restricts the top-level `providers` list.
//
// `provider_group_metric` picks what those bands measure: "additions"
// (default), which routes delete-heavy PRs to the smaller band, or "total"
// (additions + deletions). It does not affect min_diff_size/max_diff_size
// eligibility, which always measures the total. Under "additions" the smallest
// band must start at min_diff_size: 0 — a pure deletion has 0 additions, and a
// size no band covers falls back to a random pick from every enabled provider.
//
...
// { minDiffSize, maxDiffSize, providers } bands. Each band restricts which
// providers the automatic invite may pick from for a PR of that diff size
// (additions + deletions, inclusive bounds). Bands are evaluated in order and
// the first match wins; entries without usable providers are dropped.
...
// What `provider_groups` bands measure. Anything unrecognised falls back to
// the default so a typo can't silently reroute every PR.
export const PROVIDER_GROUP_METRICS = new Set(["total", "additions"]);

export function normalizeProviderGroupMetric(raw) {
  if (typeof raw !== "string") return AI_REVIEW_DEFAULTS.providerGroupMetric;

  const metric = raw.trim().toLowerCase();
  if (PROVIDER_GROUP_METRICS.has(metric)) return metric;

  console.warn(`[ai-reviews config] Ignoring unknown provider_group_metric "${raw}"`);
  return AI_REVIEW_DEFAULTS.providerGroupMetric;
}
...
export function normalizeAiReview(raw) {
  ...
  return {
    ...
    providerGroupMetric: normalizeProviderGroupMetric(raw?.provider_group_metric),
...
export async function loadAiReviewConfig(context) {
  ...
  const config = {
    ...
    // The enabled providers a PR of this band size should be reviewed by, per
    // `ai_review.provider_groups` (first matching band wins). The size is
    // measured by `ai_review.provider_group_metric` — see getProviderGroupSize.
    // Returns null when
    ...
    providersForBandSize(bandSize) {
      if (typeof bandSize !== "number" || !Number.isFinite(bandSize)) return null;

      const group = aiReview.providerGroups.find(
        (g) => bandSize >= g.minDiffSize && bandSize <= g.maxDiffSize,
      );
