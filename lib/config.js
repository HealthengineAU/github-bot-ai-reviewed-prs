...
export async function loadAiReviewConfig(context) {
  ...
  const config = {
    ...
    // The enabled providers a PR of this diff size should be reviewed by, per
    ...
    providersForDiffSize(diffSize) {
      if (typeof diffSize !== "number" || !Number.isFinite(diffSize)) return null;

      const group = aiReview.providerGroups.find(
        (g) => diffSize >= g.minDiffSize && diffSize <= g.maxDiffSize,
      );
