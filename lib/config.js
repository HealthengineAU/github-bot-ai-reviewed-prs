...
import { BOT, EFFORT } from "./ai-reviewers.js";
...
// Normalize a raw `copilot_efforts` value into a Set of known effort levels.
// Declares which Copilot effort levels the org/repo default actually makes
// available to us (GitHub has no per-request effort parameter). An unusable
// value falls back to the default.
export function normalizeCopilotEfforts(raw) {
  const known = new Set(Object.values(EFFORT));
  ...
  return efforts.size > 0 ? efforts : new Set(AI_REVIEW_DEFAULTS.copilotEfforts);
}
...
export function normalizeAiReview(raw) {
  ...
  return {
    ...
    mechanicalMaxDiffSize: toSize(
      raw?.mechanical_max_diff_size,
      AI_REVIEW_DEFAULTS.mechanicalMaxDiffSize
    ),
    // Unset means "size alone decides"; a list means every changed file must match.
    mechanicalPaths: Array.isArray(raw?.mechanical_paths)
      ? toPatterns(raw.mechanical_paths, ["**"])
      : null,
    copilotEfforts: normalizeCopilotEfforts(raw?.copilot_efforts),
    topUpLite: raw?.top_up_lite !== false,
    adviseOnExpensiveSmall: raw?.advise_on_expensive_small !== false,
