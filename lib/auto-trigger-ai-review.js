...
async function evaluateAndSummon(context, { owner, repo, pr, config }) {
  ...
  await triggerRandomReviewer(context.octokit, {
    ...
    config,
    bandSize: getProviderGroupSize(pr, config.aiReview),
  });
}
...
export function register(app) {
  app.on([
    ...
    // A PR label only waives the review when ai_review.skip_label names one.
    if (config.hasSkipLabel(pr.labels)) return;
